<!-- En caso de que form rediriga a contact/vanilla-form -->
...
if($inscripto_id){ //mientras haya inscriptops
    ...
    $data = array(); //variable con el array
    ...
    if(send_mail()){ //si se envia el mail
      /*  echo "";
        ...
        $data['result'] = 'OK'; //marcando que da ok 
    }else{
        $data['result'] = 'SEND_ERROR'; //sino va a mandar mensaje de error
    }
    
    die(json_encode($data)); //Decodifica un string de JSON
}
...
function send_mail(){
// funcion que manda email 

    $mail_to = 'Itproservices Web <webmaster@moeventos.com.ar>';  // el mail va a salir de este mail
    ...
    $subject = 'Itproservices' . $_POST{'Cata'}; // Asunto del mail
